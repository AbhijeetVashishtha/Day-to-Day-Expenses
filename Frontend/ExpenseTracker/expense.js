...
window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    let page = 1;
    await axios.get(`http://localhost:4000/user/getexpense/${page}`, {headers: {"Authorization": token}})
    .then((response) => {
...
async function getPageExpenses(page){
    ...
    const token = localStorage.getItem('token');
    const response = await axios.get(`http://localhost:4000/user/getexpense/${page}`, {headers : {"Authorization": token}});
    if(response.status === 200){
        console.log(response.data.info);
        listOfUsers.innerHTML = '';
