...
window.addEventListener('DOMContentLoaded', async(event) => {
    ...
    let page = 1;
    await axios.post(`http://localhost:4000/user/getexpense/${page}`,{Items_Per_Page: Items_Per_Page}, {headers: {"Authorization": token}})
    .then((response) => {
...
function showExpenseOnScreen(expense) {
    ...
    <button onclick="editExpense('${expense.id}', '${expense.expenseamount}', '${expense.description}', '${expense.category}')">Edit Expense</button>
...
function showPagination({currentPage,hasNextPage,hasPreviousPage,nextPage,previousPage,lastPage}){
    ...
    if(hasPreviousPage){
        ...
        button1.addEventListener('click' , ()=>getPageExpenses(previousPage, currentPage))
        pagination.appendChild(button1)
    }

    const button2 = document.createElement('button');
    button2.innerHTML = currentPage ;
    button2.addEventListener('click' , ()=>getPageExpenses(currentPage, currentPage))
    pagination.appendChild(button2)

    if(hasNextPage){
        const button3 = document.createElement('button');
        button3.innerHTML = nextPage ;
        button3.addEventListener('click' , ()=>getPageExpenses(nextPage, currentPage))
    ...
    if( currentPage!==lastPage && nextPage!==lastPage){
        ...
        button3.addEventListener('click' , ()=>getPageExpenses(lastPage, currentPage))
        pagination.appendChild(button3)
    }
...
async function getPageExpenses(page, limitper){
    ...
    const token = localStorage.getItem('token');
    const response = await axios.post(`http://localhost:4000/user/getexpense/${page}`,{Items_Per_Page: Items_Per_Page}, {headers : {"Authorization": token}});
    if(response.status === 200){
        listOfUsers.innerHTML = '';
...
function editExpense(expenseId, expamt, desc, cate){
